...
### P0-2　token 经 remote URL 传递，泄进日志与 config（第二轮深挖）

| 项 | 详情 |
|----|------|
| 现象 | `auto_push` / `_sync_main_repo` 把 token 拼进 remote URL（`https://<token>@github.com/...`）再 push |
| 泄露点 1（日志） | push 失败时 git 的 stderr 含该 URL，代码又 `log(proc.stderr[-200:])` 写盘 —— `logs/2026-06-03.log` 实测含一个 `ghp_` token |
| 泄露点 2（config） | `git remote set-url`（带 token）→ push → 还原 的写法，若中途崩溃 token 会留在 `.git/config`（origin 残留即由此而来） |
| 影响 | 旧 token 已吊销暂无害，但**机制会让下次的新 token 同样泄进本地磁盘** |
...
| P2-6 | **团队列表硬编码**：`("red", "blue")` 散布 5+ 处，将来扩队/改名要逐处改 |
...
| P0-2 | 🔴 | token 经 remote URL 传递，泄进日志/`.git config` |
